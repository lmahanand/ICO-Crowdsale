var ScratchCrowdsale = artifacts.require("ScratchCrowdsale");
var ScratchToken = artifacts.require("ScratchToken");

contract('ScratchCrowdsale', function(accounts){

    it('should deploy the token and store the address', function(done){
            ScratchCrowdsale.deployed().then(async function(instance) {
    ...
    it('one ETH should buy 5 ITCH Tokens in PreICO', function(done){
            ScratchCrowdsale.deployed().then(async function(instance) {
                const data = await instance.sendTransaction({ from: accounts[7], value: web3.toWei(1, "ether")});
                const tokenAddress = await instance.token.call();
                const scratchToken = ScratchToken.at(tokenAddress);
                const tokenAmount = await scratchToken.balanceOf(accounts[7]);
    ...
    it('one ETH should buy 2 Hashnode Tokens in ICO', function(done){
            ScratchCrowdsale.deployed().then(async function(instance) {
                ...
                const tokenAddress = await instance.token.call();
                const scratchToken = ScratchToken.at(tokenAddress);
                const tokenAmount = await scratchToken.balanceOf(accounts[2]);
